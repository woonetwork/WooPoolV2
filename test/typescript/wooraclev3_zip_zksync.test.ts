...
import { WooracleV3ZKSync, WooracleV2, TestChainLink, TestQuoteChainLink } from "../../typechain";
import WooracleV3ZKSyncArtifact from "../../artifacts/contracts/wooracle/WooracleV3ZKSync.sol/WooracleV3ZKSync.json";
...
describe("WooracleV3ZkSync", () => {
    let owner: SignerWithAddress;

    let wethToken: Contract;
    let wooToken: Contract;

    let wooracleV3Zip: WooracleV3ZKSync;
    ...
    beforeEach(async () => {
        ...
        wooracleV3Zip = (await deployContract(owner, WooracleV3ZKSyncArtifact, [])) as WooracleV3ZKSync;
        ...
        await wooracleV3Zip.setBase(5, wethToken.address);
        await wooracleV3Zip.setBase(6, wooToken.address);
    });

    it("Init states", async () => {
        expect(await wooracleV3Zip.owner()).to.eq(owner.address);
        expect(await wooracleV3Zip.getBase(0)).to.eq("0x82aF49447D8a07e3bd95BD0d56f35241523fBab1");
        expect(await wooracleV3Zip.getBase(5)).to.eq(wethToken.address);
        expect(await wooracleV3Zip.getBase(6)).to.eq(wooToken.address);

        // console.log(await wooracleV3Zip.state(wooToken.address));
        // console.log(await wooracleV3Zip.price(wethToken.address));

        const wooState = await wooracleV3Zip.state(wooToken.address);
        ...
        const price = await wooracleV3Zip.price(wethToken.address);
    ...
    it("Post prices", async () => {
        await owner.sendTransaction({
            to: wooracleV3Zip.address,
            data: _encode_woo_price_with_timestamp()
        })

        console.log(await wooracleV3Zip.state(wooToken.address));
        const p_ret = await wooracleV3Zip.price(wooToken.address)
        console.log("price ", p_ret.priceOut.toString(), p_ret.feasible);

        const ts = await wooracleV3Zip.timestamp();
        ...
        const wooInfo = await wooracleV3Zip.infos(wooToken.address);
    ...
    async function _get_id(base:String) {
        for (let i = 0; i < 5 + 2; ++i) {
            if (base == await wooracleV3Zip.getBase(i))
                return i;
        }
        return 0;
    }
